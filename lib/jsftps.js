/*
 * @package jsftps
 * @author Caleb Thompson <calebthompson@gmail.com>
 * @license https://github.com/thompsch/node-ftpsPromise/blob/master/LICENSE
 * This package is really a fork of https://github.com/sergi/jsftp. I've just made a few tweaks:
 * - added Promses 
 * - added TLS support
...
var Ftp = module.exports = function(cfg) {
  ...
  return new Promise(function(resolve, reject) {
    self.host = cfg.host || 'localhost';
    self.port = cfg.port || FTP_PORT;
    self.user = cfg.user || 'anonymous';
    self.pass = cfg.pass || '@anonymous';
    self.useList = cfg.useList || false;
    ...
    self.on('data', dbgResponse);

    self._createSocket(self.port, self.host, function() {
      self.auth(self.user, self.pass, function(e){ 
        return resolve(self) })
    });
  })
};
...
Ftp.prototype._createSocket = function(port, host, firstAction, callback) {
  //console.log('$$$$ _createSocket start')
  try {
    ...
    var connectFunction = tls.connect;
    ...
    this.pipeline.on('data', function(data) {
      //console.log('$$$ _createSocket on data')
      //console.log('$$$', data)
      self.emit('data', data);
...
Ftp.prototype._list = function(path, callback) {
  var self = this;
  var listing = '';
...
Ftp.prototype.getFile = function(remotePath) {
  //console.log('$$$ getFile start')
  var self = this
  return new Promise(function (resolve, reject) {
    var file = ''
    //console.log('$$$ getFile 2')
    self._get(remotePath, function(e,socket) {
      if (e) {
        //console.log('$$$ getFile error', e)
        return reject(e)
      }
      socket.on("data", function(d) { file += d })
      socket.on("close", function(hadErr) {
        //console.log('$$$ getFile onclose', file.length)
        //socket.end()
        //socket = undefined
        return resolve(file)
      })
...
Ftp.prototype._get = function(remotePath, localPath, callback) {
  var self = this;
  var finalCallback;
...
Ftp.prototype.getGetSocket = function(path, callback) {
  ...
  this.getPasvSocket(function(err, socket) {
    ...
    socket.on('error', function(err) {
      //console.log('$$$ getGetSocket Error', err)
    ...
    function cmdCallback(err, res) {
      if (err) {
        //console.log('$$$ cmdCallback err', err)
        return callback(err);
      }
...
Ftp.prototype._ls = function(filePath, callback) {
  ...
  if (this.useList) {
    this._list(filePath, entriesToList);
    ...
    this.raw('stat', filePath, function(err, data) {
      ...
      if (errored || isHummingbird) {
        self.useList = true;
        self._list(filePath, entriesToList);
      } else {
